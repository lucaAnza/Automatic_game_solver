...
try:
    ...
    while (consecutive_error < 100):
        ...
        if general_counter == game_iteration:
            try:
                ...
                time.sleep(2)
                webhook.webhook_print(globals.webhook_url , "Update" , "Inizio partita" , color = '03b2f8' , img_name="screenshot_partita_nuova.png")
        ...
        if (general_counter % how_much == 0):
            ...
            # Update bot discord
            webhook.webhook_print(globals.webhook_url , "Update" , f"⬤ Complimenti hai raggiunto {general_counter} iterazioni!" , color = '03b2f8' , img_name=f'screenshot_{general_counter}_iterazioni.png')
            webhookL = DiscordWebhook(url=globals.webhook_url)
    ...
    print(f"Errore{e}")
    webhook.webhook_print(globals.webhook_url , "Errore durante l'esecuzione" , f"Error: {e}" , color = 'ff0000' )
...
webhook.webhook_print(globals.webhook_url , "Checkpoint" , "Script terminated... check terminal" , color = '03b2f8' )
webhook.webhook_print(globals.webhook_url , "Update" , "TERMINAZIONE" , color = '03b2f8' , img_name='screenshot_dd_termination.png')
game_controller.close_bluestacks() # Chiude l'applicazione Bluestacks
