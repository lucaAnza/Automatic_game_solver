...
def failed_game_wh():
    webhook.webhook_print(globals.webhook_url , "Error" , "Partita non avviata per eccezione, script terminato" , color = 'ff0000')
    


def game_started_successfully():
    webhook.webhook_print(globals.webhook_url , "Success" , "Partita avviata correttamente" , color = '00ff00')


def check_error():
    print_coloured.print_cyan_ts("Possibile conflitto trovato... going to sleep")
    webhook.webhook_print(globals.webhook_url , "Intervento" , "Situazione di stallo, going to sleep" , color = 'ff0000')
    time.sleep(8)
...
def stall_fixed():
    ...
    webhook.webhook_print(globals.webhook_url , "Intervento" , "Conflitto superato... continuo a giocare! " , color = '00ff00')
    


def create_new_game():
    webhook.webhook_print(globals.webhook_url , "Iterazioni raggiunte" , "Chiusura partita attuale" , color = 'ff0000')
    ...
    webhook.webhook_print(globals.webhook_url , "Update" , "🏆 Trofei 🏆" , color = '03b2f8' , img_name = "screenshot_update_trofei.png" , trofei_screen= True)
    time.sleep(4)
